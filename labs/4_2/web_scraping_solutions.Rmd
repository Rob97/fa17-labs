...
extract_lie <- function(result){
  ...
  str_sub(lie, 2, -2) # remove quotes
}

extract_lie(first_result)
```
...
extract_explanation <- function(result) {
  explanation <- result %>% html_node(".short-truth") %>% html_text(trim = TRUE)
  str_sub(explanation, 2, -2)  
}

extract_explanation(first_result)
```
...
extract_url <- function(result) {
  result %>% html_node("a") %>% html_attr("href")  
}

extract_url(first_result)
```
